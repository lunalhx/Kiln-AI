...
public final class ExplainPromptCompiler {
    ...
    public String compile(BundleStack stack) {
        ...
        compiled.append(ExplainProfile.BASE_SYSTEM_PROMPT.trim()).append('\n');
        ...
        compiled.append('\n').append(ExplainProfile.RESPONSE_CONTRACT.trim()).append('\n');
        if (compiled.length() > INSTRUCTION_BUDGET) {
            throw new CapabilityGap("explain prompt budget exceeded: " + compiled.length());
        }
        return compiled.toString();
    }

    public String serializeContext(ExplainExecutionContext context) {
    ...
    private void validateStack(BundleStack stack) {
        for (SkillBundle bundle : stack.bundles()) {
            ...
            if (!bundle.manifest().permissions().tools().isEmpty()) {
                throw new CapabilityGap(
                        "bundle declares tools but the Explain stack is tool-free: " + bundle.pinnedId());
            }
            if (!bundle.manifest().compatibility().profiles().contains("explain")) {
                throw new CapabilityGap(
                        "bundle is not compatible with the Explain profile: " + bundle.pinnedId());
            }
            if (slot == BundleSlot.ACTION
                    && !"explain_generation/v1".equals(bundle.manifest().compatibility().responseDraft())) {
                throw new CapabilityGap(
                        "the action bundle must declare the explain_generation/v1 draft: " + bundle.pinnedId());
            }
        }
    }
}
