...
public final class ApplyPromptCompiler {
    ...
    public String compile(BundleStack stack) {
        ...
        compiled.append(ApplyProfile.BASE_SYSTEM_PROMPT.trim()).append('\n');
        ...
        compiled.append('\n').append(ApplyProfile.RESPONSE_CONTRACT.trim()).append('\n');
        if (compiled.length() > INSTRUCTION_BUDGET) {
            throw new CapabilityGap("apply prompt budget exceeded: " + compiled.length());
        }
        return compiled.toString();
    }

    public String serializeContext(ApplyExecutionContext context) {
    ...
    private void validateStack(BundleStack stack) {
        for (SkillBundle bundle : stack.bundles()) {
            ...
            if (!bundle.manifest().permissions().tools().isEmpty()) {
                throw new CapabilityGap("bundle declares tools but the Apply stack is tool-free: " + bundle.pinnedId());
            }
            if (!bundle.manifest().compatibility().profiles().contains("apply")
                    || !"apply_generation/v1".equals(bundle.manifest().compatibility().responseDraft())) {
                throw new CapabilityGap("bundle is not compatible with the Apply profile: " + bundle.pinnedId());
            }
        }
    }
}
