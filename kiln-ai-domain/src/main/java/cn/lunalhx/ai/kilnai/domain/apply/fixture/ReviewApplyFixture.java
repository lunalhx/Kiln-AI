...
/**
 * The frozen {@code kiln.task-blueprint/v1} Review Blueprint for the existing
 * polynomial-differentiation reference. It reuses the Independent Blueprint's
 * Concept, Mastery Rubric criteria, source grounding, task shape, mathematical
 * scope, notation, answer representation, and response fields, and only
 * changes the Attempt Purpose to Review, the assessment policy reference to
 * the Review final-derivative policy, and the Blueprint identity. Novelty
 * exclusions are supplied per invocation by the Review start flow and are
 * never part of the frozen template.
 */
public final class ReviewApplyFixture {

    public static final String BLUEPRINT_PINNED_ID = "apply.polynomial-differentiation.review@1.0.0";

    private ReviewApplyFixture() {
    }

    public static ApplyExecutionContext reviewContext() {
        return new ApplyExecutionContext(
                ...
                new ApplyExecutionContext.TaskBlueprint(
                        "apply.polynomial-differentiation.review",
                        "1.0.0",
