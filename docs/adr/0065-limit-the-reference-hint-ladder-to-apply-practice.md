...
The reference Hint Profile therefore serves only an open Apply Practice Task Attempt, whose private canonical expected answer permits deterministic validation of the complete H5 solution. A Teach-back Interaction Contract permits Answer Submitted, procedural Clarification Asked, and Flow Control Requested, but not Hint Requested. Diagnostic uses the same procedural-only clarification boundary. Supporting hints inside Teach-back or another Teaching Action requires a later explicit contract and verification decision; the runtime must not silently reuse the Apply Hint Ladder.
