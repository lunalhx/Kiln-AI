...
- **Knowledge source**: a university calculus textbook; title, edition, and selected scope are TBD.
- **Source input format**: TBD. PDF, Markdown, and other adapters remain undecided.
- **Target Concept**: TBD. The selected Concept must be prepared and confirmed through the normal Concept Contract rather than hardcoded into the architecture.
...
Demonstrate that one Concept grounded in the selected university calculus source can complete a learner-visible path from initial Diagnostic through adaptive learning and an Independent Test using the confirmed graph, Agent, Skill, evidence, and recovery boundaries.
...
The slice is complete when a non-hardcoded prepared calculus Concept can be selected, learned through at least one adaptive Explain/Apply/Hint path, interrupted and resumed at learner boundaries, assessed without assistance leakage, and advanced to Independent through a verified fresh task while producing a complete execution and evidence trace.
