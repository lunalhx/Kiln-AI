...
- Source Gap and Task Generation Exhausted map to the same learner message: “暂时无法准备一道可验证的题目。请稍后重试。” The learner can retry or leave the Flow. Source, model, validation, and technical details remain audit-only.
...
- The contract test must cover: valid Diagnostic task delivery; Source Gap; first-candidate verification failure followed by a valid second candidate; two failed candidates; Diagnostic passing via final derivative; Diagnostic passing via applicable rationale; Independent success with omitted/non-substantive rationale; Independent contradiction; Cannot Decide with evaluator disagreement; and least-privilege Independent context.
...
- No unresolved decision materially blocks implementation of this Apply reference. The deferred items above are explicit non-goals, not missing choices.
